...
from models.backbone.bra_legacy import *
from models.backbone._common import *
...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.stages = nn.ModuleList() # 2 feature resolution stages, each consisting of multiple residual blocks
    ...
    def forward(self, x):
        arr = []
        for i in range(4):
            x = self.downsample_layers[i](x) # res = (56, 28, 14, 7), wins = (64, 16, 2, 1)
            x = self.stages[i](x)
            arr.append(x)
            # print(x.shape)
        # x = self.norm(x)
...
if __name__ == '__main__':
    network = biformer_base(pretrained=True)
