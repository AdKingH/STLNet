...
class S3IM(torch.nn.Module):
    ...
    def forward(self, src_vec, tar_vec):
        loss = 0.0
        index_list = []
        # src_vec = self.conv(src_vec.cpu())
        # src_vec = src_vec.to('cuda')
        ...
        # img   torch.Size([24, 2, 256, 256])
        # label torch.Size([24, 256, 256])
        # print(src_all.shape, tar_all.shape)
        src_all = src_all.view(src_all.size(0), src_all.size(1), -1)
        # print(src_all.shape, tar_all.shape)
        tar_patch = tar_all.permute(1, 0, 2).reshape(128, 3, self.patch_height, self.patch_width * self.repeat_time)
        src_patch = src_all.permute(1, 0, 2).reshape(128, 3, self.patch_height, self.patch_width * self.repeat_time)
