...
def train(opt):
    ...
    # for param in model.backbone.parameters(): #冻结
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (batch_img1, batch_img2, batch_label1, batch_label2, _) in enumerate(train_tbar):
            ...
            # print(torch.unique(batch_label1))
            ...
            # out_s = scale.scale_output(out_s)
            ...
            # print(outs[0].shape, batch_label1.shape)
            # _, cd_pred1 = torch.max(outs[0], 1)  # 二值化预测结果
            # loss_s3im = criterion_s3im(out_s[0].float(), batch_label1.float())
            loss = loss_bn
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backbone", type=str, default="biformer_128")
    # parser.add_argument("--neck", type=str, default="fpn+aspp+drop")
    # parser.add_argument("--neck", type=str, default="fpn+drop")
    parser.add_argument("--neck", type=str, default="trans")
